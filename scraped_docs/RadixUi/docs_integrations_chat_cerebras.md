---
url: https://python.langchain.com/docs/integrations/chat/cerebras/
scraped_at: 2025-05-25T17:59:51.754822
title: ChatCerebras | 🦜️🔗 LangChain
---

[Skip to main content](https://python.langchain.com/docs/integrations/chat/cerebras/#__docusaurus_skipToContent_fallback)
...
[More](https://python.langchain.com/docs/integrations/chat/cerebras/)
...
[v0.3](https://python.langchain.com/docs/integrations/chat/cerebras/)
...
Search`⌘``K`
  ...
  * [Components](https://python.langchain.com/docs/integrations/components/)
    ...
    * [Document loaders](https://python.langchain.com/docs/integrations/document_loaders/)
      ...
      * [example_data](https://python.langchain.com/docs/integrations/chat/cerebras/)
      ...
      * [parsers](https://python.langchain.com/docs/integrations/chat/cerebras/)
    ...
    * [Other](https://python.langchain.com/docs/integrations/chat/cerebras/)
...
On this page
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/langchain-ai/langchain/blob/master/docs/docs/integrations/chat/cerebras.ipynb)[![Open on GitHub](https://img.shields.io/badge/Open%20on%20GitHub-grey?logo=github&logoColor=white)](https://github.com/langchain-ai/langchain/blob/master/docs/docs/integrations/chat/cerebras.ipynb)
# ChatCerebras
This notebook provides a quick overview for getting started with Cerebras [chat models](https://python.langchain.com/docs/concepts/chat_models/). For detailed documentation of all ChatCerebras features and configurations head to the [API reference](https://python.langchain.com/api_reference/cerebras/chat_models/langchain_cerebras.chat_models.ChatCerebras.html).
...
## Overview[​](https://python.langchain.com/docs/integrations/chat/cerebras/#overview "Direct link to Overview")
### Integration details[​](https://python.langchain.com/docs/integrations/chat/cerebras/#integration-details "Direct link to Integration details")
...
### Model features[​](https://python.langchain.com/docs/integrations/chat/cerebras/#model-features "Direct link to Model features")
...
---|---|---|---|---|---|---|---|---|---  
✅| ✅| ✅| ❌| ❌| ❌| ✅| ✅| ✅| ❌  
## Setup[​](https://python.langchain.com/docs/integrations/chat/cerebras/#setup "Direct link to Setup")
```
pip install langchain-cerebras
```

### Credentials[​](https://python.langchain.com/docs/integrations/chat/cerebras/#credentials "Direct link to Credentials")
...
```
import getpassimport osif"CEREBRAS_API_KEY"notin os.environ:  os.environ["CEREBRAS_API_KEY"]= getpass.getpass("Enter your Cerebras API key: ")
```
...
### Installation[​](https://python.langchain.com/docs/integrations/chat/cerebras/#installation "Direct link to Installation")
The LangChain Cerebras integration lives in the `langchain-cerebras` package:
```
%pip install -qU langchain-cerebras
```

## Instantiation[​](https://python.langchain.com/docs/integrations/chat/cerebras/#instantiation "Direct link to Instantiation")
Now we can instantiate our model object and generate chat completions:
```
from langchain_cerebras import ChatCerebrasllm = ChatCerebras(  model="llama-3.3-70b",# other params...)
```

**API Reference:**[ChatCerebras](https://python.langchain.com/api_reference/cerebras/chat_models/langchain_cerebras.chat_models.ChatCerebras.html)
## Invocation[​](https://python.langchain.com/docs/integrations/chat/cerebras/#invocation "Direct link to Invocation")
```
...
## Chaining[​](https://python.langchain.com/docs/integrations/chat/cerebras/#chaining "Direct link to Chaining")
We can [chain](https://python.langchain.com/docs/how_to/sequence/) our model with a prompt template like so:
```
from langchain_cerebras import ChatCerebrasfrom langchain_core.prompts import ChatPromptTemplatellm = ChatCerebras(  model="llama-3.3-70b",# other params...)prompt = ChatPromptTemplate.from_messages([("system","You are a helpful assistant that translates {input_language} to {output_language}.",),("human","{input}"),])chain = prompt | llmchain.invoke({"input_language":"English","output_language":"German","input":"I love programming.",})
```

**API Reference:**[ChatCerebras](https://python.langchain.com/api_reference/cerebras/chat_models/langchain_cerebras.chat_models.ChatCerebras.html) | [ChatPromptTemplate](https://python.langchain.com/api_reference/core/prompts/langchain_core.prompts.chat.ChatPromptTemplate.html)
```
...
## Streaming[​](https://python.langchain.com/docs/integrations/chat/cerebras/#streaming "Direct link to Streaming")
```
from langchain_cerebras import ChatCerebrasfrom langchain_core.prompts import ChatPromptTemplatellm = ChatCerebras(  model="llama-3.3-70b",# other params...)system ="You are an expert on animals who must answer questions in a manner that a 5 year old can understand."human ="I want to learn more about this animal: {animal}"prompt = ChatPromptTemplate.from_messages([("system", system),("human", human)])chain = prompt | llmfor chunk in chain.stream({"animal":"Lion"}):print(chunk.content, end="", flush=True)
```

**API Reference:**[ChatCerebras](https://python.langchain.com/api_reference/cerebras/chat_models/langchain_cerebras.chat_models.ChatCerebras.html) | [ChatPromptTemplate](https://python.langchain.com/api_reference/core/prompts/langchain_core.prompts.chat.ChatPromptTemplate.html)
```
OH BOY! Let me tell you all about LIONS!Lions are the kings of the jungle! They're really big and have beautiful, fluffy manes around their necks. The mane is like a big, golden crown!Lions live in groups called prides. A pride is like a big family, and the lionesses (that's what we call the female lions) take care of the babies. The lionesses are like the mommies, and they teach the babies how to hunt and play.Lions are very good at hunting. They work together to catch their food, like zebras and antelopes. They're super fast and can run really, really fast!But lions are also very sleepy. They like to take long naps in the sun, and they can sleep for up to 20 hours a day! Can you imagine sleeping that much?Lions are also very loud. They roar really loudly to talk to each other. It's like they're saying, "ROAR! I'm the king of the jungle!"And guess what? Lions are very social. They like to play and cuddle with each other. They're like big, furry teddy bears!So, that's lions! Aren't they just the coolest?
```

## Async[​](https://python.langchain.com/docs/integrations/chat/cerebras/#async "Direct link to Async")
```
from langchain_cerebras import ChatCerebrasfrom langchain_core.prompts import ChatPromptTemplatellm = ChatCerebras(  model="llama-3.3-70b",# other params...)prompt = ChatPromptTemplate.from_messages([("human","Let's play a game of opposites. What's the opposite of {topic}? Just give me the answer with no extra input.",)])chain = prompt | llmawait chain.ainvoke({"topic":"fire"})
```
...
## Related[​](https://python.langchain.com/docs/integrations/chat/cerebras/#related "Direct link to Related")
...
[Edit this page](https://github.com/langchain-ai/langchain/edit/master/docs/docs/integrations/chat/cerebras.ipynb)
#### Was this page helpful?
[PreviousAWS Bedrock](https://python.langchain.com/docs/integrations/chat/bedrock/)[NextCloudflareWorkersAI](https://python.langchain.com/docs/integrations/chat/cloudflare_workersai/)
  * [Overview](https://python.langchain.com/docs/integrations/chat/cerebras/#overview)
    * [Integration details](https://python.langchain.com/docs/integrations/chat/cerebras/#integration-details)
    * [Model features](https://python.langchain.com/docs/integrations/chat/cerebras/#model-features)
  * [Setup](https://python.langchain.com/docs/integrations/chat/cerebras/#setup)
    * [Credentials](https://python.langchain.com/docs/integrations/chat/cerebras/#credentials)
    * [Installation](https://python.langchain.com/docs/integrations/chat/cerebras/#installation)
  * [Instantiation](https://python.langchain.com/docs/integrations/chat/cerebras/#instantiation)
  * [Invocation](https://python.langchain.com/docs/integrations/chat/cerebras/#invocation)
  * [Chaining](https://python.langchain.com/docs/integrations/chat/cerebras/#chaining)
  * [Streaming](https://python.langchain.com/docs/integrations/chat/cerebras/#streaming)
  * [Async](https://python.langchain.com/docs/integrations/chat/cerebras/#async)
  * [Async Streaming](https://python.langchain.com/docs/integrations/chat/cerebras/#async-streaming)
  * [API reference](https://python.langchain.com/docs/integrations/chat/cerebras/#api-reference)
  * [Related](https://python.langchain.com/docs/integrations/chat/cerebras/#related)
