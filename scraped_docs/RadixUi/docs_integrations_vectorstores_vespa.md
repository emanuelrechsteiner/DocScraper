...
#### Creating a Vespa application[​](https://python.langchain.com/docs/integrations/vectorstores/vespa/#creating-a-vespa-application "Direct link to Creating a Vespa application")
...
#### Creating a Vespa vector store[​](https://python.langchain.com/docs/integrations/vectorstores/vespa/#creating-a-vespa-vector-store "Direct link to Creating a Vespa vector store")
Now, let's load some documents:
```
from langchain_community.document_loaders import TextLoaderfrom langchain_text_splitters import CharacterTextSplitterloader = TextLoader("../../how_to/state_of_the_union.txt")documents = loader.load()text_splitter = CharacterTextSplitter(chunk_size=1000, chunk_overlap=0)docs = text_splitter.split_documents(documents)from langchain_community.embeddings.sentence_transformer import(  SentenceTransformerEmbeddings,)embedding_function = SentenceTransformerEmbeddings(model_name="all-MiniLM-L6-v2")
```

**API Reference:**[TextLoader](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.text.TextLoader.html) | [CharacterTextSplitter](https://python.langchain.com/api_reference/text_splitters/character/langchain_text_splitters.character.CharacterTextSplitter.html) | [SentenceTransformerEmbeddings](https://python.langchain.com/api_reference/community/embeddings/langchain_community.embeddings.huggingface.SentenceTransformerEmbeddings.html)
...
**API Reference:**[VespaStore](https://python.langchain.com/api_reference/community/vectorstores/langchain_community.vectorstores.vespa.VespaStore.html)
...
#### Updating documents[​](https://python.langchain.com/docs/integrations/vectorstores/vespa/#updating-documents "Direct link to Updating documents")
...
#### Deleting documents[​](https://python.langchain.com/docs/integrations/vectorstores/vespa/#deleting-documents "Direct link to Deleting documents")
...
### Returning with scores[​](https://python.langchain.com/docs/integrations/vectorstores/vespa/#returning-with-scores "Direct link to Returning with scores")
...
### As retriever[​](https://python.langchain.com/docs/integrations/vectorstores/vespa/#as-retriever "Direct link to As retriever")
To use this vector store as a [LangChain retriever](https://python.langchain.com/docs/how_to/#retrievers) simply call the `as_retriever` function, which is a standard vector store method:
```
db = VespaStore.from_documents(docs, embedding_function, app=vespa_app,**vespa_config)retriever = db.as_retriever()query ="What did the president say about Ketanji Brown Jackson"results = retriever.invoke(query)# results[0].metadata["id"] == "id:testapp:testapp::32"
```

This allows for more general, unstructured, retrieval from the vector store.
### Metadata[​](https://python.langchain.com/docs/integrations/vectorstores/vespa/#metadata "Direct link to Metadata")
...
```
# Add metadatafor i, doc inenumerate(docs):  doc.metadata["date"]=f"2023-{(i %12)+1}-{(i %28)+1}"  doc.metadata["rating"]=range(1,6)[i %5]  doc.metadata["author"]=["Joe Biden","Unknown"][min(i,1)]
```
...
### Custom query[​](https://python.langchain.com/docs/integrations/vectorstores/vespa/#custom-query "Direct link to Custom query")
...
### Hybrid search[​](https://python.langchain.com/docs/integrations/vectorstores/vespa/#hybrid-search "Direct link to Hybrid search")
...
### Native embedders in Vespa[​](https://python.langchain.com/docs/integrations/vectorstores/vespa/#native-embedders-in-vespa "Direct link to Native embedders in Vespa")
...
### Approximate nearest neighbor[​](https://python.langchain.com/docs/integrations/vectorstores/vespa/#approximate-nearest-neighbor "Direct link to Approximate nearest neighbor")
...
## Related[​](https://python.langchain.com/docs/integrations/vectorstores/vespa/#related "Direct link to Related")
...
[Edit this page](https://github.com/langchain-ai/langchain/edit/master/docs/docs/integrations/vectorstores/vespa.ipynb)
#### Was this page helpful?
[PreviousVectara](https://python.langchain.com/docs/integrations/vectorstores/vectara/)[Nextviking DB](https://python.langchain.com/docs/integrations/vectorstores/vikingdb/)
  * [Returning with scores](https://python.langchain.com/docs/integrations/vectorstores/vespa/#returning-with-scores)
  * [As retriever](https://python.langchain.com/docs/integrations/vectorstores/vespa/#as-retriever)
  * [Metadata](https://python.langchain.com/docs/integrations/vectorstores/vespa/#metadata)
  * [Custom query](https://python.langchain.com/docs/integrations/vectorstores/vespa/#custom-query)
  * [Hybrid search](https://python.langchain.com/docs/integrations/vectorstores/vespa/#hybrid-search)
  * [Native embedders in Vespa](https://python.langchain.com/docs/integrations/vectorstores/vespa/#native-embedders-in-vespa)
  * [Approximate nearest neighbor](https://python.langchain.com/docs/integrations/vectorstores/vespa/#approximate-nearest-neighbor)
  * [Related](https://python.langchain.com/docs/integrations/vectorstores/vespa/#related)
