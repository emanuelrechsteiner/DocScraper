...
## Before You Begin[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#before-you-begin "Direct link to Before You Begin")
...
```
# @markdown Please fill in the both the Google Cloud region and name of your Cloud SQL instance.REGION ="us-central1"# @param {type:"string"}INSTANCE ="test-instance"# @param {type:"string"}# @markdown Please specify a database and a table for demo purpose.DATABASE ="test"# @param {type:"string"}TABLE_NAME ="test-default"# @param {type:"string"}
```

### 🦜🔗 Library Installation[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#-library-installation "Direct link to 🦜🔗 Library Installation")
...
### ☁ Set Your Google Cloud Project[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#-set-your-google-cloud-project "Direct link to ☁ Set Your Google Cloud Project")
...
### 🔐 Authentication[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#-authentication "Direct link to 🔐 Authentication")
...
## Basic Usage[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#basic-usage "Direct link to Basic Usage")
### MySQLEngine Connection Pool[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#mysqlengine-connection-pool "Direct link to MySQLEngine Connection Pool")
...
### Initialize a table[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#initialize-a-table "Direct link to Initialize a table")
...
### Save documents[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#save-documents "Direct link to Save documents")
...
### Load documents[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#load-documents "Direct link to Load documents")
...
```
from langchain_google_cloud_sql_mysql import MySQLLoaderloader = MySQLLoader(engine=engine, table_name=TABLE_NAME)docs = loader.lazy_load()for doc in docs:print("Loaded documents:", doc)
```

### Load documents via query[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#load-documents-via-query "Direct link to Load documents via query")
...
```
from langchain_google_cloud_sql_mysql import MySQLLoaderloader = MySQLLoader(  engine=engine,  query=f"select * from `{TABLE_NAME}` where JSON_EXTRACT(langchain_metadata, '$.fruit_id') = 1;",)onedoc = loader.load()onedoc
```

The view generated from SQL query can have different schema than default table. In such cases, the behavior of MySQLLoader is the same as loading from table with non-default schema. Please refer to section [Load documents with customized document page content & metadata](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#load-documents-with-customized-document-page-content--metadata).
### Delete documents[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#delete-documents "Direct link to Delete documents")
...
```
from langchain_google_cloud_sql_mysql import MySQLLoaderloader = MySQLLoader(engine=engine, table_name=TABLE_NAME)docs = loader.load()print("Documents before delete:", docs)saver.delete(onedoc)print("Documents after delete:", loader.load())
```

## Advanced Usage[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#advanced-usage "Direct link to Advanced Usage")
### Load documents with customized document page content & metadata[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#load-documents-with-customized-document-page-content--metadata "Direct link to Load documents with customized document page content & metadata")
First we prepare an example table with non-default schema, and populate it with some arbitrary data.
```
import sqlalchemywith engine.connect()as conn:  conn.execute(sqlalchemy.text(f"DROP TABLE IF EXISTS `{TABLE_NAME}`"))  conn.commit()  conn.execute(    sqlalchemy.text(f"""      CREATE TABLE IF NOT EXISTS `{TABLE_NAME}`(        fruit_id INT AUTO_INCREMENT PRIMARY KEY,        fruit_name VARCHAR(100) NOT NULL,        variety VARCHAR(50),        quantity_in_stock INT NOT NULL,        price_per_unit DECIMAL(6,2) NOT NULL,        organic TINYINT(1) NOT NULL      )      """))  conn.execute(    sqlalchemy.text(f"""      INSERT INTO `{TABLE_NAME}` (fruit_name, variety, quantity_in_stock, price_per_unit, organic)      VALUES        ('Apple', 'Granny Smith', 150, 0.99, 1),        ('Banana', 'Cavendish', 200, 0.59, 0),        ('Orange', 'Navel', 80, 1.29, 1);      """))  conn.commit()
```
...
```
loader = MySQLLoader(  engine=engine,  table_name=TABLE_NAME,)loader.load()
```
...
```
loader = MySQLLoader(  engine=engine,  table_name=TABLE_NAME,  content_columns=["variety","quantity_in_stock","price_per_unit","organic",],  metadata_columns=["fruit_id","fruit_name"],)loader.load()
```

### Save document with customized page content & metadata[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#save-document-with-customized-page-content--metadata "Direct link to Save document with customized page content & metadata")
...
```
test_docs =[  Document(    page_content="Granny Smith 150 0.99",    metadata={"fruit_id":1,"fruit_name":"Apple","organic":1},),]saver = MySQLDocumentSaver(  engine=engine,  table_name=TABLE_NAME,  content_column="description",  metadata_json_column="other_metadata",)saver.add_documents(test_docs)
```

```
with engine.connect()as conn:  result = conn.execute(sqlalchemy.text(f"select * from `{TABLE_NAME}`;"))print(result.keys())print(result.fetchall())
```

### Delete documents with customized page content & metadata[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#delete-documents-with-customized-page-content--metadata "Direct link to Delete documents with customized page content & metadata")
...
```
loader = MySQLLoader(engine=engine, table_name=TABLE_NAME)docs = loader.load()print("Documents before delete:", docs)saver.delete(docs)print("Documents after delete:", loader.load())
```

## Related[​](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#related "Direct link to Related")
...
[Edit this page](https://github.com/langchain-ai/langchain/edit/master/docs/docs/integrations/document_loaders/google_cloud_sql_mysql.ipynb)
#### Was this page helpful?
[PreviousGoogle Cloud SQL for SQL server](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mssql/)[NextGoogle Cloud SQL for PostgreSQL](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_pg/)
  * [Before You Begin](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#before-you-begin)
    * [🦜🔗 Library Installation](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#-library-installation)
    * [☁ Set Your Google Cloud Project](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#-set-your-google-cloud-project)
    * [🔐 Authentication](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#-authentication)
  * [Basic Usage](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#basic-usage)
    * [MySQLEngine Connection Pool](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#mysqlengine-connection-pool)
    * [Initialize a table](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#initialize-a-table)
    * [Save documents](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#save-documents)
    * [Load documents](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#load-documents)
    * [Load documents via query](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#load-documents-via-query)
    * [Delete documents](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#delete-documents)
  * [Advanced Usage](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#advanced-usage)
    * [Load documents with customized document page content & metadata](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#load-documents-with-customized-document-page-content--metadata)
    * [Save document with customized page content & metadata](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#save-document-with-customized-page-content--metadata)
    * [Delete documents with customized page content & metadata](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#delete-documents-with-customized-page-content--metadata)
  * [Related](https://python.langchain.com/docs/integrations/document_loaders/google_cloud_sql_mysql/#related)
