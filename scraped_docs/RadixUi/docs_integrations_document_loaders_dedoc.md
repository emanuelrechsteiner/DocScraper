---
url: https://python.langchain.com/docs/integrations/document_loaders/dedoc/
scraped_at: 2025-05-25T17:58:45.792057
title: Dedoc | 🦜️🔗 LangChain
---

[Skip to main content](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#__docusaurus_skipToContent_fallback)
...
[More](https://python.langchain.com/docs/integrations/document_loaders/dedoc/)
...
[v0.3](https://python.langchain.com/docs/integrations/document_loaders/dedoc/)
...
Search`⌘``K`
  ...
  * [Components](https://python.langchain.com/docs/integrations/components/)
    ...
    * [Document loaders](https://python.langchain.com/docs/integrations/document_loaders/)
      ...
      * [example_data](https://python.langchain.com/docs/integrations/document_loaders/dedoc/)
      ...
      * [parsers](https://python.langchain.com/docs/integrations/document_loaders/dedoc/)
    ...
    * [Other](https://python.langchain.com/docs/integrations/document_loaders/dedoc/)
...
On this page
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/langchain-ai/langchain/blob/master/docs/docs/integrations/document_loaders/dedoc.ipynb)[![Open on GitHub](https://img.shields.io/badge/Open%20on%20GitHub-grey?logo=github&logoColor=white)](https://github.com/langchain-ai/langchain/blob/master/docs/docs/integrations/document_loaders/dedoc.ipynb)
# Dedoc
This sample demonstrates the use of `Dedoc` in combination with `LangChain` as a `DocumentLoader`.
## Overview[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#overview "Direct link to Overview")
[Dedoc](https://dedoc.readthedocs.io) is an [open-source](https://github.com/ispras/dedoc) library/service that extracts texts, tables, attached files and document structure (e.g., titles, list items, etc.) from files of various formats.
`Dedoc` supports `DOCX`, `XLSX`, `PPTX`, `EML`, `HTML`, `PDF`, images and more. Full list of supported formats can be found [here](https://dedoc.readthedocs.io/en/latest/#id1).
### Integration details[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#integration-details "Direct link to Integration details")
...
### Loader features[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#loader-features "Direct link to Loader features")
...
DedocPDFLoader| ❌| ❌  
DedocAPIFileLoader| ❌| ❌  
## Setup[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#setup "Direct link to Setup")
  * To access `DedocFileLoader` and `DedocPDFLoader` document loaders, you'll need to install the `dedoc` integration package.
  * To access `DedocAPIFileLoader`, you'll need to run the `Dedoc` service, e.g. `Docker` container (please see [the documentation](https://dedoc.readthedocs.io/en/latest/getting_started/installation.html#install-and-run-dedoc-using-docker) for more details):


```
docker pull dedocproject/dedocdocker run -p 1231:1231
```

`Dedoc` installation instruction is given [here](https://dedoc.readthedocs.io/en/latest/getting_started/installation.html).
```
# Install package%pip install --quiet "dedoc[torch]"
```
...
## Instantiation[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#instantiation "Direct link to Instantiation")
```
from langchain_community.document_loaders import DedocFileLoaderloader = DedocFileLoader("./example_data/state_of_the_union.txt")
```

**API Reference:**[DedocFileLoader](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.dedoc.DedocFileLoader.html)
## Load[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#load "Direct link to Load")
```
docs = loader.load()docs[0].page_content[:100]
```
...
## Lazy Load[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#lazy-load "Direct link to Lazy Load")
```
docs = loader.lazy_load()for doc in docs:print(doc.page_content[:100])break
```
...
## API reference[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#api-reference "Direct link to API reference")
...
## Loading any file[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#loading-any-file "Direct link to Loading any file")
For automatic handling of any file in a [supported format](https://dedoc.readthedocs.io/en/latest/#id1), `DedocFileLoader` can be useful. The file loader automatically detects the file type with a correct extension.
File parsing process can be configured through `dedoc_kwargs` during the `DedocFileLoader` class initialization. Here the basic examples of some options usage are given, please see the documentation of `DedocFileLoader` and [dedoc documentation](https://dedoc.readthedocs.io/en/latest/parameters/parameters.html) to get more details about configuration parameters.
### Basic example[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#basic-example "Direct link to Basic example")
```
from langchain_community.document_loaders import DedocFileLoaderloader = DedocFileLoader("./example_data/state_of_the_union.txt")docs = loader.load()docs[0].page_content[:400]
```
...
### Modes of split[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#modes-of-split "Direct link to Modes of split")
...
```
loader = DedocFileLoader("./example_data/layout-parser-paper.pdf",  split="page",  pages=":2",)docs = loader.load()len(docs)
```
...
### Handling tables[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#handling-tables "Direct link to Handling tables")
...
```
loader = DedocFileLoader("./example_data/mlb_teams_2012.csv")docs = loader.load()docs[1].metadata["type"], docs[1].metadata["text_as_html"][:200]
```
...
### Handling attached files[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#handling-attached-files "Direct link to Handling attached files")
...
```
loader = DedocFileLoader("./example_data/fake-email-attachment.eml",  with_attachments=True,)docs = loader.load()docs[1].metadata["type"], docs[1].page_content
```
...
## Loading PDF file[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#loading-pdf-file "Direct link to Loading PDF file")
...
`Dedoc` can extract `PDF` with or without a textual layer, as well as automatically detect its presence and correctness. Several `PDF` handlers are available, you can use `pdf_with_text_layer` parameter to choose one of them. Please see [parameters description](https://dedoc.readthedocs.io/en/latest/parameters/pdf_handling.html) to get more details.
For `PDF` without a textual layer, `Tesseract OCR` and its language packages should be installed. In this case, [the instruction](https://dedoc.readthedocs.io/en/latest/tutorials/add_new_language.html) can be useful.
```
from langchain_community.document_loaders import DedocPDFLoaderloader = DedocPDFLoader("./example_data/layout-parser-paper.pdf", pdf_with_text_layer="true", pages="2:2")docs = loader.load()docs[0].page_content[:400]
```

**API Reference:**[DedocPDFLoader](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.DedocPDFLoader.html)
```
'\n2\n\nZ. Shen et al.\n\n37], layout detection [38, 22], table detection [26], and scene text detection [4].\n\nA generalized learning-based framework dramatically reduces the need for the\n\nmanual speciﬁcation of complicated rules, which is the status quo with traditional\n\nmethods. DL has the potential to transform DIA pipelines and beneﬁt a broad\n\nspectrum of large-scale document digitization projects.\n'
```

## Dedoc API[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#dedoc-api "Direct link to Dedoc API")
...
To use `DedocAPIFileLoader`, you should run the `Dedoc` service, e.g. `Docker` container (please see [the documentation](https://dedoc.readthedocs.io/en/latest/getting_started/installation.html#install-and-run-dedoc-using-docker) for more details):
```
docker pull dedocproject/dedocdocker run -p 1231:1231
```
...
```
from langchain_community.document_loaders import DedocAPIFileLoaderloader = DedocAPIFileLoader("./example_data/state_of_the_union.txt",  url="https://dedoc-readme.hf.space",)docs = loader.load()docs[0].page_content[:400]
```

**API Reference:**[DedocAPIFileLoader](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.dedoc.DedocAPIFileLoader.html)
```
'\nMadam Speaker, Madam Vice President, our First Lady and Second Gentleman. Members of Congress and the Cabinet. Justices of the Supreme Court. My fellow Americans. \n\n\n\nLast year COVID-19 kept us apart. This year we are finally together again. \n\n\n\nTonight, we meet as Democrats Republicans and Independents. But most importantly as Americans. \n\n\n\nWith a duty to one another to the American people to '
```

## Related[​](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#related "Direct link to Related")
...
[Edit this page](https://github.com/langchain-ai/langchain/edit/master/docs/docs/integrations/document_loaders/dedoc.ipynb)
#### Was this page helpful?
[PreviousDatadog Logs](https://python.langchain.com/docs/integrations/document_loaders/datadog_logs/)[NextDiffbot](https://python.langchain.com/docs/integrations/document_loaders/diffbot/)
  * [Overview](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#overview)
    * [Integration details](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#integration-details)
    * [Loader features](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#loader-features)
  * [Setup](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#setup)
  * [Instantiation](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#instantiation)
  * [Load](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#load)
  * [Lazy Load](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#lazy-load)
  * [API reference](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#api-reference)
  * [Loading any file](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#loading-any-file)
    * [Basic example](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#basic-example)
    * [Modes of split](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#modes-of-split)
    * [Handling tables](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#handling-tables)
    * [Handling attached files](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#handling-attached-files)
  * [Loading PDF file](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#loading-pdf-file)
  * [Dedoc API](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#dedoc-api)
  * [Related](https://python.langchain.com/docs/integrations/document_loaders/dedoc/#related)
