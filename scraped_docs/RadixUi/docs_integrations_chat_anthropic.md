...
Search`⌘``K`
  ...
  * [Components](https://python.langchain.com/docs/integrations/components/)
    ...
    * [Other](https://python.langchain.com/docs/integrations/chat/anthropic/)
...
Note that certain Anthropic models can also be accessed via AWS Bedrock and Google VertexAI. See the [ChatBedrock](https://python.langchain.com/docs/integrations/chat/bedrock/) and [ChatVertexAI](https://python.langchain.com/docs/integrations/chat/google_vertex_ai_palm/) integrations to use Anthropic models via these services.
## Overview[​](https://python.langchain.com/docs/integrations/chat/anthropic/#overview "Direct link to Overview")
### Integration details[​](https://python.langchain.com/docs/integrations/chat/anthropic/#integration-details "Direct link to Integration details")
...
### Model features[​](https://python.langchain.com/docs/integrations/chat/anthropic/#model-features "Direct link to Model features")
...
---|---|---|---|---|---|---|---|---|---  
✅| ✅| ❌| ✅| ❌| ❌| ✅| ✅| ✅| ❌  
## Setup[​](https://python.langchain.com/docs/integrations/chat/anthropic/#setup "Direct link to Setup")
...
### Credentials[​](https://python.langchain.com/docs/integrations/chat/anthropic/#credentials "Direct link to Credentials")
...
```
import getpassimport osif"ANTHROPIC_API_KEY"notin os.environ:  os.environ["ANTHROPIC_API_KEY"]= getpass.getpass("Enter your Anthropic API key: ")
```
...
### Installation[​](https://python.langchain.com/docs/integrations/chat/anthropic/#installation "Direct link to Installation")
...
## Instantiation[​](https://python.langchain.com/docs/integrations/chat/anthropic/#instantiation "Direct link to Instantiation")
...
## Invocation[​](https://python.langchain.com/docs/integrations/chat/anthropic/#invocation "Direct link to Invocation")
```
...
## Chaining[​](https://python.langchain.com/docs/integrations/chat/anthropic/#chaining "Direct link to Chaining")
...
## Content blocks[​](https://python.langchain.com/docs/integrations/chat/anthropic/#content-blocks "Direct link to Content blocks")
...
```
from pydantic import BaseModel, FieldclassGetWeather(BaseModel):"""Get the current weather in a given location"""  location:str= Field(..., description="The city and state, e.g. San Francisco, CA")llm_with_tools = llm.bind_tools([GetWeather])ai_msg = llm_with_tools.invoke("Which city is hotter today: LA or NY?")ai_msg.content
```
...
## Extended thinking[​](https://python.langchain.com/docs/integrations/chat/anthropic/#extended-thinking "Direct link to Extended thinking")
...
## Prompt caching[​](https://python.langchain.com/docs/integrations/chat/anthropic/#prompt-caching "Direct link to Prompt caching")
Anthropic supports [caching](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching) of [elements of your prompts](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching#what-can-be-cached), including messages, tool definitions, tool results, images and documents. This allows you to re-use large documents, instructions, [few-shot documents](https://python.langchain.com/docs/concepts/few_shot_prompting/), and other data to reduce latency and costs.
...
### Messages[​](https://python.langchain.com/docs/integrations/chat/anthropic/#messages "Direct link to Messages")
```
...
Below, we implement a simple chatbot that incorporates this feature. We follow the LangChain [chatbot tutorial](https://python.langchain.com/docs/tutorials/chatbot/), but add a custom [reducer](https://langchain-ai.github.io/langgraph/concepts/low_level/#reducers) that automatically marks the last content block in each user message with `cache_control`. See below:
```
import requestsfrom langchain_anthropic import ChatAnthropicfrom langgraph.checkpoint.memory import MemorySaverfrom langgraph.graph import START, StateGraph, add_messagesfrom typing_extensions import Annotated, TypedDictllm = ChatAnthropic(model="claude-3-7-sonnet-20250219")# Pull LangChain readmeget_response = requests.get("https://raw.githubusercontent.com/langchain-ai/langchain/master/README.md")readme = get_response.textdefmessages_reducer(left:list, right:list)->list:# Update last user messagefor i inrange(len(right)-1,-1,-1):if right[i].type=="human":      right[i].content[-1]["cache_control"]={"type":"ephemeral"}breakreturn add_messages(left, right)classState(TypedDict):  messages: Annotated[list, messages_reducer]workflow = StateGraph(state_schema=State)# Define the function that calls the modeldefcall_model(state: State):  response = llm.invoke(state["messages"])return{"messages":[response]}# Define the (single) node in the graphworkflow.add_edge(START,"model")workflow.add_node("model", call_model)# Add memorymemory = MemorySaver()app = workflow.compile(checkpointer=memory)
```

**API Reference:**[ChatAnthropic](https://python.langchain.com/api_reference/anthropic/chat_models/langchain_anthropic.chat_models.ChatAnthropic.html) | [MemorySaver](https://langchain-ai.github.io/langgraph/reference/checkpoints/#langgraph.checkpoint.memory.MemorySaver) | [StateGraph](https://langchain-ai.github.io/langgraph/reference/graphs/#langgraph.graph.state.StateGraph) | [add_messages](https://langchain-ai.github.io/langgraph/reference/graphs/#langgraph.graph.message.add_messages)
```
...
## Token-efficient tool use[​](https://python.langchain.com/docs/integrations/chat/anthropic/#token-efficient-tool-use "Direct link to Token-efficient tool use")
...
## Citations[​](https://python.langchain.com/docs/integrations/chat/anthropic/#citations "Direct link to Citations")
...
### Simple example[​](https://python.langchain.com/docs/integrations/chat/anthropic/#simple-example "Direct link to Simple example")
...
```
[{'text': "You can find LangChain's tutorials at https://python.langchain.com/docs/tutorials/\n\nThe tutorials section is recommended for those looking to build something specific or who prefer a hands-on learning approach. It's considered the best place to get started with LangChain.", 'type': 'text', 'citations': [{'type': 'content_block_location',  'cited_text': "[Tutorials](https://python.langchain.com/docs/tutorials/):If you're looking to build something specific orare more of a hands-on learner, check out ourtutorials. This is the best place to get started.",  'document_index': 0,  'document_title': None,  'start_block_index': 243,  'end_block_index': 248}]}]
```

## Built-in tools[​](https://python.langchain.com/docs/integrations/chat/anthropic/#built-in-tools "Direct link to Built-in tools")
Anthropic supports a variety of [built-in tools](https://docs.anthropic.com/en/docs/build-with-claude/tool-use/text-editor-tool), which can be bound to the model in the [usual way](https://python.langchain.com/docs/how_to/tool_calling/). Claude will generate tool calls adhering to its internal schema for the tool:
### Web search[​](https://python.langchain.com/docs/integrations/chat/anthropic/#web-search "Direct link to Web search")
...
**API Reference:**[ChatAnthropic](https://python.langchain.com/api_reference/anthropic/chat_models/langchain_anthropic.chat_models.ChatAnthropic.html)
### Text editor[​](https://python.langchain.com/docs/integrations/chat/anthropic/#text-editor "Direct link to Text editor")
...
## API reference[​](https://python.langchain.com/docs/integrations/chat/anthropic/#api-reference "Direct link to API reference")
...
## Related[​](https://python.langchain.com/docs/integrations/chat/anthropic/#related "Direct link to Related")
...
[PreviousAlibaba Cloud PAI EAS](https://python.langchain.com/docs/integrations/chat/alibaba_cloud_pai_eas/)[Next[Deprecated] Experimental Anthropic Tools Wrapper](https://python.langchain.com/docs/integrations/chat/anthropic_functions/)
  * [Overview](https://python.langchain.com/docs/integrations/chat/anthropic/#overview)
    * [Integration details](https://python.langchain.com/docs/integrations/chat/anthropic/#integration-details)
    * [Model features](https://python.langchain.com/docs/integrations/chat/anthropic/#model-features)
  * [Setup](https://python.langchain.com/docs/integrations/chat/anthropic/#setup)
    * [Credentials](https://python.langchain.com/docs/integrations/chat/anthropic/#credentials)
    * [Installation](https://python.langchain.com/docs/integrations/chat/anthropic/#installation)
  * [Instantiation](https://python.langchain.com/docs/integrations/chat/anthropic/#instantiation)
  * [Invocation](https://python.langchain.com/docs/integrations/chat/anthropic/#invocation)
  * [Chaining](https://python.langchain.com/docs/integrations/chat/anthropic/#chaining)
  * [Content blocks](https://python.langchain.com/docs/integrations/chat/anthropic/#content-blocks)
  * [Extended thinking](https://python.langchain.com/docs/integrations/chat/anthropic/#extended-thinking)
  * [Prompt caching](https://python.langchain.com/docs/integrations/chat/anthropic/#prompt-caching)
    * [Messages](https://python.langchain.com/docs/integrations/chat/anthropic/#messages)
    * [Tools](https://python.langchain.com/docs/integrations/chat/anthropic/#tools)
    * [Incremental caching in conversational applications](https://python.langchain.com/docs/integrations/chat/anthropic/#incremental-caching-in-conversational-applications)
  * [Token-efficient tool use](https://python.langchain.com/docs/integrations/chat/anthropic/#token-efficient-tool-use)
  * [Citations](https://python.langchain.com/docs/integrations/chat/anthropic/#citations)
    * [Simple example](https://python.langchain.com/docs/integrations/chat/anthropic/#simple-example)
    * [Using with text splitters](https://python.langchain.com/docs/integrations/chat/anthropic/#using-with-text-splitters)
  * [Built-in tools](https://python.langchain.com/docs/integrations/chat/anthropic/#built-in-tools)
    * [Web search](https://python.langchain.com/docs/integrations/chat/anthropic/#web-search)
    * [Text editor](https://python.langchain.com/docs/integrations/chat/anthropic/#text-editor)
  * [API reference](https://python.langchain.com/docs/integrations/chat/anthropic/#api-reference)
  * [Related](https://python.langchain.com/docs/integrations/chat/anthropic/#related)
