...
scraped_at: 2025-05-25T08:35:09.110613
...
Getting Started
  ...
  3. [Nuxt](https://supabase.com/docs/guides/getting-started/quickstarts/nuxtjs)
...
1
2
SUPABASE_URL=<SUBSTITUTE_SUPABASE_URL>SUPABASE_ANON_KEY=<SUBSTITUTE_SUPABASE_ANON_KEY>
...
20
21
<scriptsetup>import{createClient}from'@supabase/supabase-js'constconfig=useRuntimeConfig()constsupabase=createClient(config.public.supabaseUrl,config.public.supabaseAnonKey)constinstruments=ref([])asyncfunctiongetInstruments(){const{data}=awaitsupabase.from('instruments').select()instruments.value=data}onMounted(()=>{getInstruments()})</script><template><ul><li v-for="instrument in instruments":key="instrument.id">{{ instrument.name }}</li></ul></template>
